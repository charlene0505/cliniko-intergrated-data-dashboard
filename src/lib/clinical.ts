...
export interface CaseRecord {
  _id: string; patientId: string; name: string; isEpc: boolean;
...
export function isEpcCase(name: string) { return name.toUpperCase().includes('GPCCMP'); }
export function summarizeClinical(cases: CaseRecord[], attendances: AttendanceRecord[], appointments: AppointmentRecord[], now = new Date()) {
  ...
  const plans: EpcPlan[] = cases.filter(c => isEpcCase(c.name)).map(c => {
